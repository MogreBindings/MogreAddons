		...
		<xsl:for-each select="memberdef[@kind='enum']">
			...
			<xsl:if test="starts-with(@id,../../@id)">
				<xsl:element name="enumeration">
					...
					<!-- PIPAAAAA-->
					<xsl:attribute name="protection" select="@prot"/>
					<xsl:attribute name="includeFile" select="substring-after(location/@file,'/ois/includes/')"/>
				...
				<xsl:choose>
					<xsl:when test="contains(type,'std::') or contains(type,',')">
						...
						<xsl:choose>
							...
							<xsl:when test="starts-with($string1,'std::') and starts-with($string1,'std::string')=false">
