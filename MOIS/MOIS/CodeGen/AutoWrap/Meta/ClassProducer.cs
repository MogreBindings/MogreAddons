...
namespace AutoWrap.Meta
{
    class ClassProducer : Producer 
    {
        ...
        protected List<DefClass> _listenersByListenerSetter = new List<DefClass>();
        ...
        protected virtual void Init()
        {
            ...
            {
                if (f.IsListenerAdder)
                {
                    _listeners.Add((DefClass)f.Parameters[0].Type);
                }
                else if (f.IsListenerSetter)
                {
                    _listenersByListenerSetter.Add((DefClass)f.Parameters[0].Type);
                }
            }
        ...
        {
            return (_t.BaseClass == null || _t.BaseClass.HasWrapType(WrapTypes.NativeDirector)) ? GetTopBaseClassName() : _t.BaseClass.CLRName;
        }
        ...
        protected virtual void AddAllNestedTypes()
        {
            ...
            foreach (DefType nested in typedefs)
            {
