...
public:																				\
	...
	public:																			\
		...
		{																			\
			private: virtual System::Object^ get() sealed = Collections::IEnumerator::Current::get;	\
		}																			\
																					\
		virtual void Reset()														\
		{																			\
	...
	public:																			\
		...
		{																			\
			if ((System::Object^)it1 == (System::Object^)it2) return true;							\
			if ((System::Object^)it1 == nullptr || (System::Object^)it2 == nullptr) return false;	\
																					\
...
public:																				\
	...
	int CountValues(MK key);														\
																					\
	MOIS::Pair<Iterator^, Iterator^> EqualRange(MK key);							\
																					\
...
public:																				\
	...
	int CountValues(MK key);														\
																					\
	MOIS::Pair<ConstIterator^, ConstIterator^> EqualRange(MK key);					\
																					\
	Collections::Generic::List<MV>^ GetValues(MK key);								\
};
...
#define CPP_DECLARE_STLMULTIMAP_NONCONST( PREFIX, CLASS_NAME, MK, MV, NK, NV )				\
	bool PREFIX##CLASS_NAME::Enumerator::MoveNext()											\
		...
		System::Object^ PREFIX##CLASS_NAME::Enumerator::NonGenericCurrent::get()					\
			{																		\
				return Collections::Generic::KeyValuePair<MK,MV>(ToManaged<MK,NK>( (*_pIter)->first ), ToManaged<MV,NV>( (*_pIter)->second ));	\
			}																		\
																					\
	...
	PREFIX##Const_##CLASS_NAME^ PREFIX##CLASS_NAME::ReadOnlyInstance::get()			\
		{																			\
			if (_readOnly == (System::Object^)nullptr)										\
	...
	{																				\
		return _native->count(ToNative<MK,NK>( key ));								\
																					...
																					\
	MOIS::Pair<PREFIX##CLASS_NAME::Iterator^, PREFIX##CLASS_NAME::Iterator^> PREFIX##CLASS_NAME::EqualRange(MK key)				\
	{																				\
		std::pair<Iter,Iter> range = _native->equal_range(ToNative<MK,NK>( key ));	\
		return MOIS::Pair<PREFIX##CLASS_NAME::Iterator^, PREFIX##CLASS_NAME::Iterator^>(range.first, range.second);	\
	}																				\
																					\
...
#define CPP_DECLARE_STLMULTIMAP_CONST( PREFIX, CLASS_NAME, MK, MV, NK, NV )						\
	bool PREFIX##Const_##CLASS_NAME::Enumerator::MoveNext()									\
		...
		System::Object^ PREFIX##Const_##CLASS_NAME::Enumerator::NonGenericCurrent::get()				\
			{																		\
				return Collections::Generic::KeyValuePair<MK,MV>(ToManaged<MK,NK>( (*_pIter)->first ), ToManaged<MV,NV>( (*_pIter)->second ));	\
			}																		\
																					\
	...
	{																				\
		return _native->count(ToNative<MK,NK>( key ));								\
																					...
																					\
	MOIS::Pair<PREFIX##Const_##CLASS_NAME::ConstIterator^, PREFIX##Const_##CLASS_NAME::ConstIterator^> PREFIX##Const_##CLASS_NAME::EqualRange(MK key)	\
	{																				\
		std::pair<Iter,Iter> range = _native->equal_range(ToNative<MK,NK>( key ));	\
		return MOIS::Pair<PREFIX##Const_##CLASS_NAME::ConstIterator^, PREFIX##Const_##CLASS_NAME::ConstIterator^>(range.first, range.second);	\
	}																				\
																					\
