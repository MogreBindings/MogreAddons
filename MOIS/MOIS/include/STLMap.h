...
public:																				\
	...
	void Erase(MK key);																\
																					\
	MOIS::Pair<Iterator^, bool> Insert(MK key, MV value);							\
																					\
...
#define CPP_DECLARE_STLMAP_NONCONST( PREFIX, CLASS_NAME, MK, MV, NK, NV )				\
	...
	{																				\
		_native->erase(ToNative<MK,NK>( key ));										\
																					...
																					\
	MOIS::Pair<PREFIX##CLASS_NAME::Iterator^, bool> PREFIX##CLASS_NAME::Insert(MK key, MV value)	\
	{																				\
		std::pair<Iter,bool> res = _native->insert( std::pair<NK,NV>( ToNative<MK,NK>( key ), ToNative<MV,NV>( value ) ) );	\
		return MOIS::Pair<PREFIX##CLASS_NAME::Iterator^, bool>(res.first, res.second);		\
	}																				\
																					\
