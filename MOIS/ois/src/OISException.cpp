...
//Additions for Mogre
Exception LastExceptionThrown((OIS_ERROR)-1, "", 0, "");

Exception* Exception::getLastException(void) throw()
{
  if (LastExceptionThrown.eType == (OIS_ERROR)-1)
    return 0;
...
{
  LastExceptionThrown = Exception((OIS_ERROR)-1, "", 0, "");
}

Exception::Exception( OIS_ERROR err, const char* str, int line, const char *file )
